...
public class DefaultDependencyLockingProvider implements DependencyLockingProvider {
    ...
    @Override
    public void persistResolvedDependencies(String configurationName, Set<ModuleComponentIdentifier> resolvedModules, boolean containsChangingModule) {
        if (writeLocks) {
            ...
            if (containsChangingModule) {
                LOGGER.warn("Dependency lock state for configuration '{}' contains changing modules. This means that dependencies content may still change over time. See {} for details.", configurationName, DOC_REG.getDocumentationFor("dependency_locking"));
            }
            if (LOGGER.isDebugEnabled()) {
